...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT * from users where user_id=$from";
    $query = mysqli_query($conn, $sql);
    $sql1 = mysqli_fetch_array($query);

    $sql = "SELECT * from users where user_id=$to";
    $query = mysqli_query($conn, $sql);
    ...
    if (($amount) < 0) {
        ...
        $sql = "UPDATE users SET `Current Balance`=$newbalance WHERE user_id=$from";
        mysqli_query($conn, $sql);

        // adding amount to receiver's account
        $newbalance = $sql2['Current Balance'] + $amount;
        $sql = "UPDATE users SET `Current Balance`=$newbalance WHERE user_id=$to";
        mysqli_query($conn, $sql);
            ...
            <select name="to" class="form-control" required>
                ...
                ?>
                    <option class="table" value="<?php echo $rows['user_id']; ?>">
                        <?php echo $rows['Name']; ?> (<?php echo $rows['Current Balance']; ?>)
                    </option>
                <?php
                }
                ?>
